...
def main():
    ...
    # 6. Fit Readout weights using Ridge Regression
    print("Solving Ridge Regression for Readout weights...")
    # W_out = Y^T X (X^T X + beta * I)^-1
    # We solve: (X^T X + beta * I) W_out^T = X^T Y
    ...
    print("\n--- Generating Text Sample (Temp=0.25) ---")
    print(f"Prompt: {prompt}", end="")
    ...
    for _ in range(150):
        ...
        probs = torch.softmax(out / 0.25, dim=0).numpy()
